            ...
            if (!$modelImage->image->name = $dataImage) {
                if ($model->validate()) {
                   (new UploadApi())->upload($modelImage->image->tempName, [
                        'public_id' => 'portfolio/' . $imageBasename,
                        'quality' => 40,
                        'resource_type' => 'image'
                    ]);
                    ...
                    $modelImage->path = 'portfolio/' . $imageBasename;
                    $modelImage->image_url = $imageHost . '/' . $imageName;
                    $modelImage->save();

                    $model->image_id = $modelImage->id;
                    $model->path = 'portfolio/' . $imageBasename;
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())){
            ...
            if(!empty($modelImage->image)){
                ...
                $searchImage_id = Portfolio::find($imageId)->where(['image_id' => $model->image_id])->all();
                ...
                if (!$modelImage->image->name = $dataImage) {
                    if ($model->validate()){
                        ...
                        if ($model->image_id = $searchImage_id && $getCount >= 2) {
                            ...
                            $newModelImage->path = 'portfolio/' . $imageBasename;
